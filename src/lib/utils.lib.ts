...
export const formatPrice = (price: string, prices: Prices) => {
  ...
  const slicedPriceFirst = price.slice(0, price.length - currency_minor_unit);
  const slicedPriceLast = price.slice(-currency_minor_unit);

  // Join the sliced parts with the decimal separator
  const formattedPrice = slicedPriceFirst + currency_decimal_separator + slicedPriceLast;
